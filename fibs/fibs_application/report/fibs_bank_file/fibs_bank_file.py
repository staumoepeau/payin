...
SIA = "SIA%"
BSP_Bank = "BSP%"
MBF_Bank = ["MBF", "MBF01", "MBF02","MBF03","MBF04", "MBF05"]
TDB_Bank = "TDB%"
FEDERAL = "FEDERAL%"
...
def get_data(filters):
	...
	other = frappe.db.sql("""SELECT sal.employee, sal.employee_name,
			IF(ded.salary_component LIKE %s, "BSP",
				IF(ded.salary_component IN %s, "MBF",
					IF(ded.salary_component LIKE %s, "TDB",
						IF(ded.salary_component LIKE %s, "FEDERAL",
			...
			AND sal.company = %s
			HAVING bankname = %s
			""", (BSP_Bank, MBF_Bank, TDB_Bank, FEDERAL, SIA, postingdate, company, bankname), as_dict=1)
	
	entry = frappe.db.sql("""select employee, employee_name, net_pay as amount, 
		bank_account_no as account_number, bank_name, company, department
		from `tabSalary Slip`
		where docstatus = 1
		and net_pay > 0 
		AND posting_date = %s
	...
	employee = {}
	for o in other:
		if not o.account_number:
				o.account_number = ''
		
		if bankname == 'TDB' or bankname == 'MBF':
			description = o.description
			emp_name = o.employee_name

			if o.description:
				o.employee_name = description
				o.description = emp_name.upper() 
			else:
				o.description = ''
#				o.employee_name = emp_name
		if bankname == 'BSP':
			if o.description:
				o.description = o.description.upper() 
			else:
				o.description = ''

		employee = {
			"employee_name" : o.employee_name.upper(),
			"employee": o.employee,
			"amount" : o.amount,
...
def get_bank_data(postingdate, company, bankname):
	...
	employee_list = frappe.db.sql("""SELECT sal.employee, sal.employee_name, 
			ded.salary_component AS bankname, ded.amount, deb.account_number, ded.description
			FROM `tabSalary Slip` sal
			INNER JOIN `tabSalary Detail` ded ON
				sal.name = ded.parent
			AND ded.parentfield = 'deductions'
			...
			IF(ded.salary_component LIKE %s, "BSP",
				IF(ded.salary_component IN %s, "MBF",
					IF(ded.salary_component LIKE %s, "TDB",
						IF(ded.salary_component LIKE %s, "FEDERAL",
							IF(ded.salary_component LIKE %s, "SIA", ded.salary_component))))) AS bankname,
			ded.amount, deb.account_number, ded.description
			FROM `tabSalary Slip` sal
			INNER JOIN `tabSalary Detail` ded ON
				sal.name = ded.parent
			AND ded.parentfield = 'deductions'
	...
	entry = frappe.db.sql("""SELECT employee, employee_name, net_pay as amount, 
		account_number, bank_name, company, department
		FROM `tabSalary Slip`
		WHERE net_pay > 0
		AND posting_date = %s
		AND company = %s
...
#				d.description = ''
		...
		#	"payment_details" : d.payment_details,
			"bank_name" : d.bank_name,
			"account_number": d.account_number.replace("-",""),
	...
	SIA_Finance = frappe.db.sql("""SELECT sal.employee, 
		IF(ded.salary_component = "SIA Finance","SIA Finance", sal.employee_name) AS employee_name, 
			IF(ded.salary_component = "SIA Finance", "BSP", ded.salary_component) AS bankname,
			SUM(ded.amount) AS amount, deb.account_number as account_number, ded.description
			FROM `tabSalary Slip` sal
			INNER JOIN `tabSalary Detail` ded ON sal.name = ded.parent
			AND ded.parentfield = 'deductions'
			AND ded.parenttype = 'Salary Slip'
...
def get_sum_account(posting_date, company, bank_name):
	...
	sum_account_1 = frappe.db.sql("""select sum(account_number)
		from `tabSalary Slip`
		where docstatus = 1
		and net_pay != 0
		and posting_date = %s

...
frappe.query_reports["FIBS PAYE"] = {
	"filters": [{
		fieldname: "company",
		label: __("Company"),
		fieldtype: "Link",
...
onload: function(report) {
	...
	report.page.set_primary_action('PAYE', function() {
		var args = "as a draft"
		var reporter = frappe.query_reports["FIBS PAYE"];
		reporter.maketextfile(report);
	})

	return frappe.call({
		method: "fibs.fibs_application.report.fibs_paye.fibs_paye.get_years",
		callback: function(r) {
...
maketextfile: function(report) {
	...
	if (filters.company) {
		return frappe.call({
			method: "fibs.fibs_application.report.fibs_paye.fibs_paye.save_data_to_Excel",
			args: {
				"month": filters.month,
